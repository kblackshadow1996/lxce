        ...
        <div class="row">
            <div id="left" class="col-md-4 col-xs-12">
                <ul class="left-container wow bounceInLeft">
                    <p>公司产品</p>
                    <li :class="(index+1)==$route.query.type?'active':''" v-for="(item,index) in softwareList" :key="index">
                        <router-link :to="'/software?type='+(index+1)">{{item.name}}</router-link>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    data(){
        return{
            softwareList: this.$store.state.categoryList
        }
    },
    mounted() {
        var wow = new WOW();
